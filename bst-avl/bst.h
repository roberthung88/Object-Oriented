...
protected:
    ...
    void removeHelper(Node<Key, Value>* found);
...
BinarySearchTree<Key, Value>::iterator::operator++()
{
    if(current_->getRight() != NULL){
        Node<Key, Value> *curr_ = current_->getRight();
        while(curr_->getLeft() != NULL){
            curr_ = curr_->getLeft();
        }
        current_ = curr_;
        return *this;
        ...
        Node<Key, Value>* temp = curr->getParent();
        while(temp != NULL && curr == temp->getRight())
        {
            curr = temp;
            temp = temp->getParent();
        }
    ...
    while(curr_ != NULL){
        if(keyValuePair.first > curr_->getKey()){
            if(curr_->getRight() == NULL){
                Node<Key, Value>* toInsert = new Node<Key, Value>(keyValuePair.first, keyValuePair.second, curr_);
...
template<typename Key, typename Value>
void BinarySearchTree<Key, Value>::removeHelper(Node<Key, Value>* found){
    if(found != NULL){
        if(found->getLeft() != NULL && found->getRight() != NULL){
            //has both children
            Node<Key, Value> *pred = predecessor(found);
            nodeSwap(pred, found);
            removeHelper(found);
        }else if(found->getLeft() != NULL){
            //only has left child
            Node<Key, Value> *left = found->getLeft();
            if(found != root_){
            ...
            Node<Key, Value> *right = found->getRight();
            if(found != root_){
                if(found->getParent()->getLeft() == found){
...
BinarySearchTree<Key, Value>::predecessor(Node<Key, Value>* current)
{
    if(current->getLeft() != NULL){
        Node<Key, Value> *curr_ = current->getLeft();
        while(curr_->getRight() != NULL){
            curr_ = curr_->getRight();
        }
        return curr_;
    }else{
        Node<Key, Value> *curr_ = current->getParent();
        
        while(curr_ != NULL){
            Node<Key, Value> *ancestor_ = curr_->getParent();
            if(ancestor_ == NULL){
                return nullptr;
            }else{
                if(ancestor_->getRight()->getKey() == curr_->getKey()){
                    return ancestor_;
                }else{
                    curr_ = ancestor_;
                }
...
template<typename Key, typename Value>
void BinarySearchTree<Key, Value>::deleteTree(Node<Key, Value>* curr){
...
int BinarySearchTree<Key, Value>::height(Node<Key, Value>* node) const{
    ...
    return 1 + std::max(height(node->getLeft()),
                   height(node->getRight()));
}

template<typename Key, typename Value>
bool BinarySearchTree<Key, Value>::balancedHelper(Node<Key, Value>* node) const{
    int lh;
    int rh;
    ...
    if (abs(lh - rh) <= 1 && balancedHelper(node->getLeft()) && balancedHelper(node->getRight())){
        return 1;
    }
