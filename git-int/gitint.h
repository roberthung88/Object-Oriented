...
public:
    ...
    bool valid_file(std::string name) const;
...
private:
    ...
    std::map<std::string,int> curr_files;
    std::map<std::string,int> edited_files;
    std::set<std::string> staged_files;
    std::vector<CommitObj> commits_;
    //maps don't hold order
    std::vector<std::string> tag_name;
    std::map<std::string,int> taghelper;
    int head_ = 0;
};
